...
void tut_display()
{
	glClear(GL_COLOR_BUFFER_BIT);	// Begining of frame (overwrites last frame)
	{
		...
		glBegin(GL_TRIANGLES);
			...
			glVertex3f(-0.5, 0.0, 0.0);	// Create a geometrix point

			glColor3f(0.0, 1.0, 0.0);		// Color, 3D in floats
			glVertex3f(-0.25, 0.5, 0.0);		// Create a geometrix point

			glColor3f(0.0, 0.0, 1.0);		// Color, 3D in floats
			glVertex3f(0, 0, 0.0);		// Create a geometrix point
		glEnd();
	}
